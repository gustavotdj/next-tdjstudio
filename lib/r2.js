...
export const R2_BUCKET_NAME = process.env.R2_BUCKET_NAME || process.env.NEXT_PUBLIC_R2_BUCKET_NAME;
export const R2_PUBLIC_URL = process.env.R2_PUBLIC_URL || process.env.NEXT_PUBLIC_R2_PUBLIC_URL;
...
export function getR2Client() {
  ...
  if (!accountId) {
    console.error("❌ CRITICAL: R2_ACCOUNT_ID is not defined!");
    // Return null or throw to prevent malformed URL
    throw new Error("R2_ACCOUNT_ID is missing from environment variables");
  }
...
// Backward compatibility export (deprecated, use getR2Client instead)
// Note: This might still be undefined at module load time if env vars are missing
export const r2 = new S3Client({
  region: 'auto',
  endpoint: `https://${process.env.R2_ACCOUNT_ID || 'MISSING_ID'}.r2.cloudflarestorage.com`,
  credentials: {
